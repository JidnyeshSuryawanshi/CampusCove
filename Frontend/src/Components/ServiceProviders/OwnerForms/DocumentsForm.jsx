...
import { FaFileAlt, FaUpload, FaTrash, FaCheckCircle, FaTimesCircle, FaFileImage, FaFilePdf, FaEye, FaExternalLinkAlt, FaTimes, FaSpinner, FaExclamationTriangle, FaIdCard, FaBuilding, FaFileContract } from 'react-icons/fa';
...
export default function DocumentsForm({ ownerId, initialData, onDocumentsUpdated, onSubmit, loading: externalLoading }) {
  ...
  useEffect(() => {
    if (initialData && initialData.documents) {
      ...
      // Handle the case where initialData is just the documents array
  ...
  const uploadDocument = async () => {
    ...
    try {
      ...
      // Upload to server using the api utility function
      ...
      if (response.data.success) {
        ...
        setDocumentName('');
        resetFileInput();
        setDocumentType('identity'); // Reset to default
        
        // Add the new document to the local state to avoid a full reload
        if (response.data.data) {
          setDocuments(prevDocs => [...prevDocs, response.data.data]);
        }
  ...
  const confirmDeleteDocument = async () => {
    ...
    try {
      setIsDeleting(true);
      const response = await api.delete(`/owner/profile/documents/${documentToDelete._id}`);
  ...
  const documentTypes = [
    ...
    { value: 'property_ownership', label: 'Property Ownership/Lease Document' },
        ...
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="E.g., Aadhar Card, Business License"
                  disabled={uploading}
                />
              </div>
            </div>
          ...
          <div className="mt-8 p-4 bg-blue-50 border border-blue-100 rounded-md">
            ...
            <p className="text-sm text-blue-800">
              After uploading all required documents, our team will verify your information within 1-2 business days.
              You will receive an email notification once your account is verified, after which you can start listing your properties.
            </p>
          </div>
          ...
          <div className="mt-8 flex justify-end">
            <button
              type="submit"
              onClick={() => onSubmit && onSubmit(documents)}
